        ...
        <nav className='box-border w-dvw fixed top-0 left-0 px-10 flex justify-center z-10 backdrop-blur-md bg-(--bg-color)/60'>
            <div className='w-full w-max-view box-border py-6 flex items-center gap-8'>
                ...
                <div className='flex grow justify-start gap-8 h-full'>
                    {
                        navItems.map( (n, idx) => (
                            <div key={idx} className={`cursor-pointer content-center title text-md hover:text-(--txt-accent-color) transition-colors ${appContext.navSelect.startsWith(n) ? 'text-(--txt-accent-color)' : 'text-(--txt-inactive-color)'}`} onClick={() => handleNavClick(n)}>{n}</div>
                        ))
                    }
                </div>
